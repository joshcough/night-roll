...
**💬 Message Claude**
File → Message Claude… opens a box that sends straight to the live Claude Code session — feature requests and bug reports from inside the app, auto-tagged with the open song and playhead position. Replies and deploy notices stream back as 🛰 lines in the info strip. Powered by two private ntfy topics you set once in File → Settings (they stay on this device only — the public repo never contains them; without them the feature is simply inert). The topic names are the password: keep them secret.
