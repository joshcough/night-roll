...
**💬 Message Claude**
File → Message Claude… opens a box that sends straight to the live Claude Code session — feature requests and bug reports from inside the app, auto-tagged with the open song and playhead position — with a 🎤 Speak button, so a request is: open, talk, Send. Replies and deploy notices stream back as 🛰 lines in the info strip AND light a gold 🛰 badge in the footer (like the ⚠ error button) that stays lit until you tap it and read — with a Reply button right in the list. Powered by two private ntfy topics you set once in File → Settings (they stay on this device only — the public repo never contains them; without them the feature is simply inert). The topic names are the password: keep them secret.
