...
**▦ Grid**
View ▾ → **Grid…** opens a sheet: tap a number of **lines per bar** (say 10) and the roll redraws instantly with that many lines in each bar — and pencil entry, note drags and resizes all SNAP to those lines. Pure view-and-snap aid: no note moves, no annotation changes, the time signature stays what it is. This is how a figure the duration buttons can't spell — ten even notes across a 4/4 bar, each 0.4 beats — becomes pencil-drag from line to line; with a grid armed, a pencil tap lays down one cell. **The grid follows the cycle**: drag a range on the ruler and the cells run from the cycle's START — so a figure beginning mid-bar (14.2, say) gets its lines exactly where its notes go, continuing bar after bar on that phase. Bar lines stay drawn for orientation but stop being snap targets while the grid is armed; a bar line falling between cells is expected, not a bug. No cycle = cells anchor to each bar line. **Off** returns to the meter's own grid (16ths, or triplet steps while a T duration is armed). Transient by design: set it, enter the passage, set it back.
