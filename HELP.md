...
**Edit menu**
An **Edit ▾** menu appears beside File on songs you can edit: ⟲ Undo, ⟳ Redo, ⧉ Copy, 📋 Paste at cursor, 🗑 Delete, ✂ Split, ⇅ Transpose (chromatic, octave, and in-key moves live in its sheet), ♫ Insert chord, ⇄ Move to track, 🥁 Drum pattern, 🎲 Drummer, ➗ Divide, ⁀ Join — the same actions as the edit-row icons, labeled. The row itself keeps the heavy-use tools up front and folds the rest behind **⋯** (tap to expand; the choice sticks per device) — so pencil mode's duration picker fits on one iPad row again. Redo replays what Undo took back; making any new edit clears the Redo history (standard editor behavior).
